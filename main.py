...
# ============================================================
# --------- PAGE: TREND ANALYSIS & INSIGHTS ------------------
# ============================================================
elif page == "Trend Analysis and Insights":
    st.header("📉 Trend Analysis & Insights")

    if len(numeric_cols) == 0:
        st.error("No numeric columns available for trend analysis.")
    else:
        target = st.selectbox("Select variable for trend analysis:", numeric_cols, key="trend_target")
        df_trend = df_pivot[["Timestamp", target]].dropna()
        if df_trend.empty or len(df_trend) < 2:
            st.info("Not enough data for trend analysis.")
        else:
            df_trend["ts_num"] = df_trend["Timestamp"].astype(np.int64) // 10**9
            model = LinearRegression()
            model.fit(df_trend[["ts_num"]], df_trend[target])
            slope = model.coef_[0]
            intercept = model.intercept_
            future_seconds = df_trend["ts_num"].max() + 24 * 3600
            forecast_value = model.predict([[future_seconds]])[0]
            df_trend["Trend"] = model.predict(df_trend[["ts_num"]])
            if slope > 0:
                trend_text = f"📈 **Increasing trend** detected (slope = {slope:.4f})"
            elif slope < 0:
                trend_text = f"📉 **Decreasing trend** detected (slope = {slope:.4f})"
            else:
                trend_text = "➖ **Stable / No significant trend** detected"
            st.info(trend_text)
            fig = px.line(df_trend, x="Timestamp", y=[target, "Trend"], title=f"Trend Analysis for {target}")
            st.plotly_chart(fig, use_container_width=True)
            st.success(f"🔮 **Forecast for next 24 hours:** {forecast_value:.2f} {target}")
